...
Four numbered Jupyter notebooks implement the full pipeline: data ingestion, descriptive analysis, modelling, and evaluation. Four ISLR/ESL topics are exercised:
...
FIT files contain per-second records of heart rate (bpm), cadence (strides/min), speed (m/s), GPS coordinates (lat/lon), and elevation (m). Strava-stripped GPX files retain only lat/lon/elevation/time.

\texttt{01\_ingest.ipynb} decompresses each source file in memory and rewrites it once as a Garmin-style \emph{extended GPX~1.1} (with HR and cadence inside the standard \texttt{gpxtpx:TrackPointExtension} block). All downstream notebooks read only these canonical GPX intermediates; the raw export is never touched again.
...
The 2024-04-14 Bonn Half Marathon was recorded without an HR strap: the FIT file contains no \texttt{heart\_rate} samples, and the Strava-stripped GPX exposes no cumulative distance field. To keep this race label as the sixth LORO fold, two manual rescues are applied at ingestion and feature-build time: (i) per-activity distance falls back to $\int v\,dt$ from the speed channel when \texttt{dist\_m} is unusable, and (ii) HR features (\texttt{mean\_hr}, \texttt{p50\_hr}, \texttt{p90\_hr}, \texttt{frac\_z1--z5}, \texttt{hr\_load}) are imputed from the median of the 28~days of running activities preceding the race. The \texttt{effort} field for the row is set to 1.00 by construction (races are 100\,\%-effort observations). All six races thus form the leave-one-race-out (LORO) folds.
...
\subsection{Data Quality: Confidence Weights}
\label{sec:conf_weights}

GPS signal loss at the beginning (or during) a run is a real data quality issue: a missing GPS lock produces NaN speed and elevation for the affected seconds, making pace and GAP unreliable for those observations. Rather than applying a binary drop/keep threshold — which would reduce an already small training set — each activity is assigned a continuous confidence score $w_i \in [0, 1]$ that down-weights unreliable pseudo-labels in proportion to their quality:

\begin{equation}
w_i = c_{\text{GPS},i} \times c_{\text{ele},i} \times c_{\text{HR},i} \times c_{\text{dur},i}
\end{equation}

where:
\begin{itemize}
  \item $c_{\text{GPS}} = \text{fraction of seconds with valid GPS speed}$
  \item $c_{\text{ele}} = \text{fraction of seconds with valid elevation (needed for GAP)}$
  \item $c_{\text{HR}}  = \text{fraction of seconds with valid HR (drives the effort estimate)}$
  \item $c_{\text{dur}} = \min(1,\, t_{\text{moving}} / 1800\,\text{s})$ — full weight at $\geq 30$ min; ramps linearly below
\end{itemize}

Race rows are assigned $w = 1.0$ unconditionally: they are ground-truth labels regardless of sensor quality. The weight $w_i$ is then passed as \texttt{sample\_weight} to every model's \texttt{fit()} call (GAM, RF, GBR, SVR) and as a per-sample multiplier on the squared error in the CNN's loss function. Activities are never discarded; they simply contribute proportionally less to the learned surface when their data quality is lower.
...
SVR achieves the lowest mean absolute percentage error across the six LORO folds (17.97\,\%), with the tabular field tightly clustered: GAM~19.02\,\%, RF~18.96\,\%, GBR~19.17\,\%. Riegel produces predictions for only four of the six folds (the first two races have no prior race available), so its 7.96\,\% MAPE is computed on a smaller, easier subset. Summary statistics over six data points remain noisy; the results are better read race by race.

The two short races (5--5.6\,km) show consistently large over-predictions from the tabular models ($+$20--43\,\%), while the 10\,km, half marathon, and marathon predictions are substantially closer (mostly within $\pm$5\,\% on the half and the marathon). This asymmetry has a structural explanation: the training distribution is dominated by 10--20\,km runs at moderate effort; the short races at near-maximal intensity are near the edge of the support, and all tabular models regress toward the training mean.

SVR is the closest to the observed marathon time (only $+$0.31\,\% over-prediction on 2026-04-12). The half marathon (2024-04-14, with imputed HR features) is also predicted within $\pm$15\,\% across all four tabular models, validating the imputation strategy.

The Riegel formula, which requires no fitting and sees only a single prior race, achieves $-0.72\,\%$ on the marathon and $+3.32\,\%$ on the 5.555\,km Lousberglauf, but underperforms on the 10\,km and 5\,km folds where the population-level exponent~1.06 does not match this athlete's individual speed-endurance curve.
...
\begin{enumerate}
  \item \textbf{Six race labels.} LORO is the correct validation strategy with so few labels, but six points cannot establish absolute calibration. Confidence intervals on the errors are wide.
  ...
  \item \textbf{Bonn Half Marathon HR is imputed.} The 2024-04-14 HM was recorded without an HR strap; HR features are filled from the 28-day pre-race median (see Data Exception above). LORO error on this fold reflects both the model and the imputation; an HR-equipped HM would tighten the half-marathon-distance evaluation.
\end{enumerate}
...
This project demonstrates that a single athlete's GPS and heart-rate training history, when processed through a grade-adjusted pace pipeline and a pseudo-label inversion framework, provides a reasonable basis for marathon finish-time prediction. SVR achieves the lowest mean absolute percentage error across the six leave-one-race-out folds (17.97\,\%) and is the closest model on the marathon itself ($+$0.31\,\%). The Riegel formula, despite its simplicity, achieves $-0.72\,\%$ on the marathon while failing on shorter distances. A daily-cadence ``race-time predictor'' visualisation (Notebook~04, \S4.7) reproduces the user-facing curves shown by Garmin and Strava across configurable distances and models, demonstrating that the same temporal-honesty pipeline can drive a real-time predictor in addition to the LORO evaluation.
...
Four Jupyter notebooks implement the full pipeline (run in order from the \texttt{Project/} directory):
...
\begin{description}
  ...
  \item[\texttt{03\_models.ipynb}] Builds the feature table; implements HR imputation and the speed-integration distance fallback for HR-less race rows; runs GAM / RF+GBR / SVR / 1D-CNN with LORO; reports the results in Table~\ref{tab:loro}; persists \texttt{data/processed/loro\_results.parquet} and \texttt{data/processed/feature\_importance.parquet}.
  \item[\texttt{04\_evaluation.ipynb}] Loads the LORO and feature-importance parquets; computes aggregate metrics (MAPE/MAE/MedAE/RMSE/Bias) and the model-comparison plots (\S4.2--4.3); reports the GBR 90\,\% prediction-interval coverage (\S4.4); plots Random-Forest / GBR feature importance (\S4.5); applies a Minetti-formula course-elevation correction to a chosen race (\S4.6, filename-stem matched against \texttt{data/courses/*.gpx}); and produces a daily-cadence Garmin/Strava-style race-time predictor across configurable distances, models and lookback windows (\S4.7).
\end{description}
